...
execute if data storage mh_dp:status Monster.Win{Reus:true}

# data modify entity @s Offers.Recipes append value {maxUses:1000,xp:0,buy:{id:"minecraft:clock",Count:1b,tag:{display:{Name:'{"text":"竜骨"}',Lore:['{"text":"頑強な竜の骨．様々な用途がある．"}']},CustomModelData:10018,MhdpID:19}},sell:{id:"minecraft:crossbow",Count:1b,tag:{display:{Name:'{"text":"竜骨笛"}',Lore:['{"text":"大きな音を鳴らし，"}','{"text":"縄張り内のモンスターを刺激する．"}','{"text":"一度使うと無くなる．"}']},CustomModelData:10000,MhdpItem:1,HideFlags:32,ChargedProjectiles:[{id:"minecraft:air",Count:1b},{},{}],Charged:1b}}}
# 
# execute at @s run summon wandering_trader ~ ~ ~ {Tags:["MhdpTrader"]}
# execute at @s run data modify entity @e[type=wandering_trader,tag=MhdpTrader,distance=..1,limit=1] Offers set from entity @s Offers 
