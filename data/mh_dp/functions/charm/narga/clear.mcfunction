...
# メインハンド or オフハンドを検知してアイテム置き換え
    execute if data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].PlayerData.Weapon.tag{MhdpCharm:6} run tag @s add Cleared
    execute if entity @s[tag=Cleared] run item replace entity @s weapon.mainhand with air
    execute unless entity @s[tag=Cleared] if data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].PlayerData.Offhand.tag{MhdpCharm:6} run item replace entity @s weapon.offhand with air
