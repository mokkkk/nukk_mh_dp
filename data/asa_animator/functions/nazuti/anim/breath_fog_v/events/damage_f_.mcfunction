...
execute if score #mhdp_nazuti_temp AsaMatrix matches 9.. run particle dust 0.643 0.322 0.706 3 ^ ^ ^ 1.5 1.5 1.5 0.05 4

execute if score #mhdp_nazuti_temp AsaMatrix matches 4.. positioned ^ ^ ^-2 as @e[type=!armor_stand,type=!area_effect_cloud,type=!marker,tag=!NazutiParts,tag=!MhdpHit,distance=..3.5] run tag @s add MhdpDHit

execute if score #mhdp_nazuti_temp AsaMatrix matches ..15 positioned ^ ^ ^1 if block ~ ~ ~ #asa_animator:no_collision run function asa_animator:nazuti/anim/breath_fog_v/events/damage_f_