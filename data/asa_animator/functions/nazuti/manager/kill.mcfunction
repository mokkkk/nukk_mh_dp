...
# 討伐タイマー計算
scoreboard players remove #mhdp_nazuti_kill_timer AsaMatrix 118
execute store result score #mhdp_min AsaMatrix store result score #mhdp_sec AsaMatrix run scoreboard players get #mhdp_nazuti_kill_timer AsaMatrix
...
# パーティクル
execute at @e[type=armor_stand,tag=NazutiParts] run particle poof ~ ~1 ~ 1 1 1 0 30

# アイテムドロップ
execute positioned ~ ~5 ~ run function asa_animator:nazuti/manager/loot
