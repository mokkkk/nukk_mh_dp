...
# 討伐タイマー計算
scoreboard players remove #mhdp_dyno_kill_timer AsaMatrix 90
execute store result score #mhdp_min AsaMatrix store result score #mhdp_sec AsaMatrix run scoreboard players get #mhdp_dyno_kill_timer AsaMatrix
...
# パーティクル
execute at @e[type=armor_stand,tag=DynoParts] run particle poof ~ ~1 ~ 1 1 1 0 30

# アイテムドロップ
execute positioned ~ ~5 ~ run function asa_animator:dyno/manager/loot

# 進捗達成
advancement grant @a[distance=0..64] only mh_dp:trophies/root
advancement grant @a[distance=0..64] only mh_dp:trophies/dyno
data modify storage mh_dp:status Monster.Win.Dyno set value true
