...
# 尻尾煤化
execute if entity @s[tag=IsHeatTail] unless entity @s[tag=IsSootTail] if score #mhdp_dyno_actcount_generic AsaMatrix matches 5.. run function asa_animator:dyno/manager/model/change_to_soot_tail
# 頭部赤熱化
execute if entity @s[tag=IsSootTail] if score #mhdp_dyno_actcount_generic AsaMatrix matches 1.. run tag @s add AnmCharge
# 頭部赤熱化終了
execute if entity @s[tag=IsHeatHead] if score #mhdp_dyno_actcount_head AsaMatrix matches 4.. run function asa_animator:dyno/manager/model/change_to_normal_head
