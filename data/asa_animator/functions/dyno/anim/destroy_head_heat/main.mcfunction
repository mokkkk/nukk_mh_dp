...
execute if score @s AsaMatrix matches 56.. run function asa_animator:dyno/anim/destroy_head_heat/end
...
execute if score @s AsaMatrix matches 1 run scoreboard players set #mhdp_dyno_head_heat_damage AsaMatrix 0

execute if score @s AsaMatrix matches 9 run function asa_animator:dyno/manager/model/change_to_normal_head
execute if score @s AsaMatrix matches 9 as @e[distance=0..15,type=armor_stand,tag=DynoParts,tag=HeadU] at @s run particle explosion_emitter ~ ~1.4 ~ 0 0 0 0 1
execute if score @s AsaMatrix matches 9 run playsound entity.generic.explode master @a ~ ~ ~ 2 0.7