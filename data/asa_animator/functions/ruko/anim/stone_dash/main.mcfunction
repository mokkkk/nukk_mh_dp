...
execute if entity @s[scores={AsaMatrix=1}] run function asa_animator:ruko/anim/dash/start
execute if entity @s[scores={AsaMatrix=1}] run function asa_animator:ruko/anim/dash/keyframes/0
execute if entity @s[scores={AsaMatrix=1..4}] run tp @s ^0 ^0.1 ^0
execute if entity @s[scores={AsaMatrix=5}] run function asa_animator:ruko/anim/dash/keyframes/1
execute if entity @s[scores={AsaMatrix=5..9}] run tp @s ^0 ^0.12 ^0
execute if entity @s[scores={AsaMatrix=10}] run function asa_animator:ruko/anim/dash/keyframes/2
execute if entity @s[scores={AsaMatrix=10..14}] run tp @s ^0 ^-0.04 ^0
execute if entity @s[scores={AsaMatrix=15}] run function asa_animator:ruko/anim/dash/keyframes/3
execute if entity @s[scores={AsaMatrix=15..18}] run tp @s ^0 ^-0.2 ^0
execute if entity @s[scores={AsaMatrix=19..}] run function asa_animator:ruko/anim/stone_dash/end
execute as @e[type=armor_stand,tag=RukoParts] run function #asa_matrix:animate
function asa_animator:ruko/model

execute as @e[type=marker,tag=RukoMoveRotate] rotated as @s run function asa_animator:ruko/anim/stone_dash_start/events/rotate
...
# 停止
execute if entity @e[type=marker,tag=RukoDashTarget,distance=0..10] run scoreboard players set #mhdp_ruko_dash_count AsaMatrix 2
execute if entity @e[type=marker,tag=RukoDashTarget,distance=0..10] run kill @e[type=marker,tag=RukoDashTarget,distance=0..10]

execute positioned ^ ^ ^3 run function asa_animator:ruko/anim/dash/events/damage
execute at @e[type=armor_stand,tag=RukoStoneD,distance=0..10] run function asa_animator:ruko/anim/stone_dash/events/damage