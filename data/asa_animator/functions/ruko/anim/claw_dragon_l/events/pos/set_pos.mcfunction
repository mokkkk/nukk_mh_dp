...
# 地上にmarkerセット
summon marker ^ ^ ^3 {Tags:["RukoMovePos"]}
execute facing entity @e[tag=RukoAttackTarget,limit=1] eyes rotated ~ 0 as @e[tag=RukoAttackTarget] positioned as @s run tp @e[type=marker,tag=RukoMovePos] ^ ^ ^-4.5
...
scoreboard players set #ruko_height_target AsaMatrix 6
scoreboard players operation #ruko_mine_x AsaMatrix /= #ruko_height_target AsaMatrix
scoreboard players operation #ruko_mine_y AsaMatrix /= #ruko_height_target AsaMatrix
scoreboard players operation #ruko_mine_z AsaMatrix /= #ruko_height_target AsaMatrix
