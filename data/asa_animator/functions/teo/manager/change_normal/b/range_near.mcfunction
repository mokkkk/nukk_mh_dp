...
# ランダム
loot spawn ~ 0 ~ loot asa_animator:teo/b/near
## 共通
    # バックステップ
...
## LV1 -
    ...
    # 薙ぎ払い火炎放射
    execute if entity @e[type=item,y=0] if entity @e[type=item,nbt={Item:{tag:{Act:9}}}] run tag @s add AnmFlameLarge
    # 近接コンボ
    execute if entity @e[type=item,y=0] if entity @e[type=item,nbt={Item:{tag:{Act:10}}}] run function asa_animator:teo/manager/change_normal/_/combo
    # コンボBA
    execute if entity @e[type=item,y=0] if entity @e[type=item,nbt={Item:{tag:{Act:11}}}] run function asa_animator:teo/manager/change_combo/start_ba
    # コンボBB
    execute if entity @e[type=item,y=0] if entity @e[type=item,nbt={Item:{tag:{Act:12}}}] run function asa_animator:teo/manager/change_combo/start_bb
## Lv2 -
    # 粉塵まき散らし
    execute if entity @e[type=item,y=0] if entity @e[type=item,nbt={Item:{tag:{Act:13}}}] run tag @s add AnmBombDashStart
    # 粉塵爆発（バックステップ）
    execute if entity @e[type=item,y=0] if entity @e[type=item,nbt={Item:{tag:{Act:14}}}] run tag @s add AnmBombStep
