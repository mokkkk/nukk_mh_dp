...
execute as @e[type=!armor_stand,type=!area_effect_cloud,type=!marker,tag=!NargaParts,distance=4..10] unless entity @s[tag=CharmTiga,tag=!BlightDragon] unless entity @s[scores={MhdpTAvoid=1..}] run function asa_animator:narga/anim/voice/events/damage_sub_