...
execute facing entity @e[limit=1,tag=TigaAttackTarget] feet rotated ~ 0 positioned 0 0 0 run summon marker ^ ^ ^1 {Tags:["MhdpRotater","MhdpPos1"]}
...
# 左右確認
execute unless entity @s[tag=NoRotate] facing entity @e[limit=1,tag=TigaAttackTarget] feet positioned ^ ^ ^3.0 rotated as @s positioned ^4.0 ^ ^ if entity @s[distance=..5.0] run tag @s add Right
