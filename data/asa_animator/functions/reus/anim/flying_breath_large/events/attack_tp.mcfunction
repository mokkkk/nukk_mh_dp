...
execute positioned ~ ~ ~ run particle smoke ~ ~ ~ 0.5 0.5 0.5 0.1 1 force
execute positioned ~ ~ ~ if entity @e[tag=ReusTarget,distance=0..2] run scoreboard players set @s AsaMatrix 0
execute if score @s AsaMatrix matches 1.. if block ~ ~ ~ #asa_animator:no_collision positioned ^ ^ ^1 run function asa_animator:reus/anim/flying_breath_large/events/attack_tp
