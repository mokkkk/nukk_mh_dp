...
# 地上にmarkerセット
summon marker ^ ^ ^10 {Tags:["BrachyMovePos"]}
execute facing entity @e[tag=BrachyAttackTarget,limit=1] eyes rotated ~ 0 as @e[tag=BrachyAttackTarget] positioned as @s run tp @e[type=marker,tag=BrachyMovePos] ^ ^ ^-2
...
execute as @e[type=marker,tag=BrachyMovePos,limit=1] at @s run tp @s ~ ~-0.2 ~
...
scoreboard players set #brachy_height_target AsaMatrix 20
scoreboard players operation #brachy_mine_x AsaMatrix /= #brachy_height_target AsaMatrix
scoreboard players operation #brachy_mine_y AsaMatrix /= #brachy_height_target AsaMatrix
scoreboard players operation #brachy_mine_z AsaMatrix /= #brachy_height_target AsaMatrix
