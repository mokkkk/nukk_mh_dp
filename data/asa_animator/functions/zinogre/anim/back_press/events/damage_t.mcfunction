...
execute if data storage mh_dp:settings Custom{QuestRank:1} run data modify storage score_damage: Argument set value {Damage:8.0f, BypassArmor:1b}
data modify storage mhdp: DamageType set value {Epf:1,Knockback:1}
execute as @e[type=!armor_stand,type=!area_effect_cloud,type=!marker,tag=!ZinogreParts,distance=..3] run function asa_animator:zinogre/anim/back_press/events/damage_sub
