...
# 始動：飛び掛かり
    execute if entity @s[tag=StartAnmJump] run tag @s add AnmTR2CHandR

# ランダム
    execute if predicate asa_animator:zinogre/stay run loot spawn ~ 0 ~ loot asa_animator:zinogre/attack/tackle
    ## 超帯電
        # チャージお手
            execute if entity @e[type=item,y=0] if entity @e[type=item,nbt={Item:{tag:{Act:1}}}] run tag @s add AnmTR2CHandR
        # サマーソルト
            execute if entity @e[type=item,y=0] if entity @e[type=item,nbt={Item:{tag:{Act:2}}}] run tag @s add AnmSaultR
