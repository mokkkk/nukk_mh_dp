...
execute if score #mhdp_brachyr_head_damage AsaMatrix matches ..0 as @e[type=armor_stand,tag=BrachyRRoot,distance=0..15] unless entity @s[tag=DestroyHead] run function asa_animator:brachyr/manager/destroy_parts/head
execute if score #mhdp_brachyr_arml_damage AsaMatrix matches ..0 as @e[type=armor_stand,tag=BrachyRRoot,distance=0..15] unless entity @s[tag=DestroyArmL] run function asa_animator:brachyr/manager/destroy_parts/arm_l
execute if score #mhdp_brachyr_armr_damage AsaMatrix matches ..0 as @e[type=armor_stand,tag=BrachyRRoot,distance=0..15] unless entity @s[tag=DestroyArmR] run function asa_animator:brachyr/manager/destroy_parts/arm_r
execute if score #mhdp_brachyr_tail_damage AsaMatrix matches ..0 as @e[type=armor_stand,tag=BrachyRRoot,distance=0..15] unless entity @s[tag=DestroyTail] run function asa_animator:brachyr/manager/destroy_parts/tail
