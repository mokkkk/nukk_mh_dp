...
# 左右確認
execute facing entity @e[limit=1,tag=BrachyRAttackTarget] feet positioned ^ ^ ^3.0 rotated as @s positioned ^4.0 ^ ^ if entity @s[distance=..5.0] run tag @s add Right

execute if entity @s[tag=NoRotate] run tag @s add AnmPunchLargeR
...
tag @s remove CurrentPunchL
tag @s add CurrentPunchR
tag @s add Punched