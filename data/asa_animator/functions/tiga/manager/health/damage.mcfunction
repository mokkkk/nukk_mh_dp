...
execute if score #mhdp_tiga_head_damage AsaMatrix matches ..0 as @e[distance=0..15,type=armor_stand,tag=TigaRoot] unless entity @s[tag=DestroyHead] run function asa_animator:tiga/manager/destroy_parts/head
execute if score #mhdp_tiga_arml_damage AsaMatrix matches ..0 as @e[distance=0..15,type=armor_stand,tag=TigaRoot] unless entity @s[tag=DestroyArmL] run function asa_animator:tiga/manager/destroy_parts/arm_l
execute if score #mhdp_tiga_armr_damage AsaMatrix matches ..0 as @e[distance=0..15,type=armor_stand,tag=TigaRoot] unless entity @s[tag=DestroyArmR] run function asa_animator:tiga/manager/destroy_parts/arm_r
execute if score #mhdp_tiga_tail_damage AsaMatrix matches ..0 as @e[distance=0..15,type=armor_stand,tag=TigaRoot] unless entity @s[tag=DestroyTail] run function asa_animator:tiga/manager/destroy_parts/tail

# 怒り遷移
scoreboard players operation #mhdp_tiga_anger_damage AsaMatrix += #tiga_damage AsaMatrix
execute if score #mhdp_tiga_anger_damage AsaMatrix matches ..-15000 unless entity @e[type=armor_stand,tag=TigaRoot,tag=IsAnger] as @e[type=armor_stand,tag=TigaRoot] run function asa_animator:tiga/manager/start_anger
