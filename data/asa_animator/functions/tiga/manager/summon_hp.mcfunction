...
# 遭遇履歴更新
    ...
    data modify storage mh_dp:status Monster.Meet.Tiga set value true

# HPセット
    execute if data storage mh_dp:settings Custom{QuestRank:0} run scoreboard players set #mhdp_tiga_health AsaMatrix 52000
    execute if data storage mh_dp:settings Custom{QuestRank:1} run scoreboard players set #mhdp_tiga_health AsaMatrix 72000
# HP倍率適用
...
# ボスバー設定
    ...
    execute if data storage mh_dp:settings Custom{QuestRank:1} run bossbar set asa_animator:tiga_health color red
