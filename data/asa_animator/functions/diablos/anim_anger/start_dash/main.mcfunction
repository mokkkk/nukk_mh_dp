...
# 突進位置決定
execute if entity @s[scores={AsaMatrix=24}] run summon marker ^ ^ ^10 {Tags:["DiablosDashTarget"]}
execute if entity @s[scores={AsaMatrix=24}] positioned as @e[tag=DiablosAttackTarget] rotated ~ 0 positioned ^ ^ ^3 run tp @e[type=marker,tag=DiablosDashTarget] ~ ~ ~
execute if entity @s[scores={AsaMatrix=24}] as @e[type=marker,tag=DiablosDashTarget] at @s run function asa_animator:zinogre/manager/check_ground
