...
#> Temp
# @within score_damage_mhdp:core/get_default_epf/**
    #declare score_holder $Temp

# 負荷軽減のためstorageに入れる
    data modify storage score_damage: Protection set value []
    execute if entity @s[type=player] run function score_damage_mhdp:core/get_default_epf/from_player
...
# ダメージ軽減の加算
    ...
    # 3
        ...
        scoreboard players operation $EPF ScoreDamageCore += $Temp ScoreDamageCore
# 各種エンチャントの加算
    execute if data storage score_damage: Argument{DamageType:"Fire"} run function score_damage_mhdp:core/get_default_epf/enchant/fire
    execute if data storage score_damage: Argument{DamageType:"Blast"} run function score_damage_mhdp:core/get_default_epf/enchant/blast
    execute if data storage score_damage: Argument{DamageType:"Projectile"} run function score_damage_mhdp:core/get_default_epf/enchant/projectile
    execute if data storage score_damage: Argument{DamageType:"Fall"} run function score_damage_mhdp:core/get_default_epf/enchant/fall
# リセット
